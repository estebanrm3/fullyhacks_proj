...
const FACE_CHECK_MS     = 1500    // face presence check every 1.5s
const PHONE_CHECK_MS    = 3_000   // Gemini phone scan every 3s for faster stop response
...
export function useWebcamProctor({ enabled = true }) {
  ...
  // Manual kill-switch — exposed so the UI button can call it
  ...
  return { videoRef, status, awayMs, cameraAways, phoneDetections, permissionDenied, isBeeping, stopAllBeeping }
}
