...
function Visualizer({ analysers, coachState }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      const mid = w / 2
      const userActive = coachState.status === 'listening' && !coachState.muted && !coachState.paused
...
function StatusLine({ state }) {
  let label = 'Tap the coach to start'
  let dot = 'idle'
  if (state.status === 'connecting') { label = 'Surfacing your coach…'; dot = 'connecting' }
  else if (state.status === 'error')  { label = state.error || 'Voice coach unavailable'; dot = 'error' }
  else if (state.status === 'closed') { label = 'Coach offline';   dot = 'idle' }
  else if (state.paused)              { label = 'Paused';          dot = 'paused' }
  else if (state.status === 'speaking'){ label = 'Coach is speaking'; dot = 'coach' }
...
export default function VoiceCoach({ analysis, workTextRef }) {
  ...
  const [state, setState]   = useState({ status: 'idle', muted: false, paused: false, error: null })
  ...
  const active = state.status === 'listening' || state.status === 'speaking' || state.status === 'connecting'
  ...
  return (
    <div className={'vc-root' + (open ? ' open' : '')}>
      <button
        ...
        title={active ? 'End voice session' : 'Start voice tutor'}
      >
        <span className="vc-launch-ring" />
        <svg className="vc-launch-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round">
          <path d="M12 3a3 3 0 0 0-3 3v6a3 3 0 0 0 6 0V6a3 3 0 0 0-3-3z" />
          <path d="M5 11a7 7 0 0 0 14 0" />
          <path d="M12 18v3" />
        </svg>
        <span className="vc-launch-label">
          {active ? 'END COACH' : 'VOICE TUTOR'}
        </span>
      </button>
      ...
      {open && (
        <div className="vc-panel" role="dialog" aria-label="Voice coach">
          <div className="vc-panel-head">
            ...
            <button
              className="vc-close"
              onClick={() => { coachRef.current?.stop(); coachRef.current = null; setOpen(false) }}
          ...
          <div className="vc-controls">
            ...
            <button
              ...
              onClick={() => { coachRef.current?.stop(); coachRef.current = null }}
              disabled={!active}
            >
              END
            </button>
